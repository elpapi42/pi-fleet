...
Commands print human-readable output. `pif send` confirms Pi accepted or queued an instruction. It does not wait for the work to finish. Use `pif --help` or `pif COMMAND --help` for generated usage. By default, pi-fleet stores its LMDB state and IPC sockets in `~/.pi-fleet`. Pre-stable releases do not migrate state from earlier locations automatically. This CLI requires Node.js 22.12 or later. Slice 2 supports Unix-like hosts with ZeroMQ `ipc://` support. It provides `create`, `list`, `status`, and `send`. Event streaming, recovery, retirement, compact, and destroy come in later slices.
