...
Running agents keep the worker version used at creation. After updating pi-fleet, create a new agent before testing new worker behavior such as worker recovery. CLI `0.16.1` uses SDK `0.12.1`, which removes `work.interrupted` from replay. Destroy and recreate agents made by older versions before using this release.
