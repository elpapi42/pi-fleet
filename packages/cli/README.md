...
Commands print human-readable output. `pif send` confirms Pi accepted or queued an instruction. It does not wait for the work to finish. `pif receive` prints durable activity and an opaque `Cursor:` line after every event. Copy a cursor into `--after` to replay later events and continue live. Use `--from-start` to replay from the first event. It shows model-requested tool parameters and bounded final tool output. It removes terminal control sequences, omits raw image data, and marks shortened or omitted values. Tool details can still contain sensitive local data. Press Ctrl-C to stop receiving. Use `pif --help` or `pif COMMAND --help` for generated usage.
