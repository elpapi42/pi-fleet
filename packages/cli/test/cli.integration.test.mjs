...
test("recovers status, send, and receive after worker replacement", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-fleet-cli-worker-recovery-"))
  const home = join(root, "home")
  const stateDir = join(home, ".pi-fleet")
  const env = {
    ...process.env,
  ...
  try {
    ...
    createdId = created.stdout.match(/^ID: (.+)$/m)?.[1]
    assert.ok(createdId)
    const beforeStore = await openStore(stateDir)
    const before = beforeStore.getById(createdId)
