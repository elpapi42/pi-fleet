...
The SDK, but not `pif`, can add immutable environment overrides to Pi child processes with `client.create({ env })`. Pi-fleet persists these plaintext values in LMDB and applies them on initial startup, Pi recovery, and worker recovery. It merges the inherited worker environment, then `env`, then `agentDir` for `PI_CODING_AGENT_DIR`. Empty strings override inherited values. `PATH` and `PI_CODING_AGENT_DIR` are reserved, and no unset or update operation exists. Pi and processes it starts can inherit values. The feature is trusted local process configuration, not a sandbox or secret store. Pi-fleet does not directly add environment configuration to status, list, or events, but Pi and tools can read values and include them in activity output. Do not print secret values or actual user-supplied values in validation errors, logs, or test output. Use only harmless placeholders in documentation examples. Destroy removes the logical record but cannot securely erase old LMDB pages or backups.
