...
- Production fixes now distinguish receive timeout from connection cancellation, reject all current and future store calls after SQLite-worker failure, clean pending Pi requests after write failures, redact unexpected public errors and Pi stderr, serialize create/send/destroy per agent, safely resume send operations when no send row was committed, and return stable receive errors when destroy/interruption wins.
- The tag publishing workflow runs all tests through `npm test`; a separate nightly Linux workflow runs faults, process, compatibility, and store benchmark suites.
